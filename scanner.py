import requests
import sqlite3

# -- Set up SQL Database --
conn = sqlite3.connect('scan_results.db')
cursor = conn.cursor()
# Create a table to store results if it doesn't exist yet
cursor.execute('''Create table if not exists scans
                    (url TEXT, payload TEXT, status TEXT)''')
conn.commit()
conn.close()

def check_sql_injection(url):
    # Re-open connection to perform the work
    conn = sqlite3.connect('scan_results.db')
    cursor = conn.cursor()

    # 1. Load payloads from the file
    with open("payloads.txt", "r") as file:
        payloads = file.read().splitlines()

    # 2. Loop through each 'poison' character
    for p in payloads:
        test_url = f"{url}{p}"
        print(f"Testing: {test_url}")

        try:
            response = requests.get(test_url)

            # 3. Look for SQL error messages in the website's response
            if "sql syntax" in response.text.lower() or "mysql" in response.text.lower():
                print(f"[!] VULNERABILITY FOUND: {p}")

                # -- Insert the finding into the SQL database --
                cursor.execute("INSERT INTO scans (url, payload, status) VALUES (?, ?, ?)",
                               (url, p, "VULNERABLE"))
                conn.commit()

                conn.close() #close the connection before leaving
                return #stop if one is found
        except:
            print("Error: Could not connect to site.")

    print("Scan complete. No obvious vulnerabilities found.")
    conn.close() #close connection at the end

# The target (a site for testing)
target = "http://testphp.vulnweb.com/artists.php?artist="
check_sql_injection(target)
